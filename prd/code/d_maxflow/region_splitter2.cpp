...
int region_splitter2::local_node(int u){
	return nodes[u].local_index;
};

int region_splitter2::get_nR(){
	/*int nR=1;
...
//region_splitter2::region_splitter2(const std::string & filename, region_graph * _G, const exttype::intf & sslice):G(_G){
region_splitter2::region_splitter2(const std::string& filename, region_graph * _G, int num_blocks, const std::vector<uint16_t>& blocks) : G(_G) {
	//slice = sslice;
...
int region_splitter2::region(int v){//!< which region is v
...
void region_splitter2::write_arc(int r,int lu, int lv, int cap1, int cap2){
...
void region_splitter2::reset_counters(){
...
void region_splitter2::allocate1(int n ,int m, int S, int T, int d, int * Sz){
	//this->S = S;
	//this->T = T;
	G->S = S;
	G->T = T;
...
void region_splitter2::arc_loop1(int u,int v){
	++G->nE;
	int r = region(u);
	int q = region(v);
...
void region_splitter2::allocate2(int loop){
	if(loop==0){
...
void region_splitter2::arc_loop2(int u,int v,int cap1, int cap2){
	++G->nE;
	int r = region(u);
	int q = region(v);
	if(r==q){//inner edge
...
void region_splitter2::allocate3(){
	for(int r=0;r<G->nR;++r){
		(*G)[r].nE_out = 0;
	};
	G->nB = 0;
...
void region_splitter2::read_arc(int loop,int u,int v,int cap1, int cap2){
	if(loop==0){
