...
template<class defs> seed_hi_pr<defs>::seed_hi_pr(){
	c1.start();c1.pause();
	c2.start();c2.pause();
	c3.start();c3.pause();
};
...
*/
/*
template<class defs> __forceinline bool seed_hi_pr<defs>::is_out_bnd(arc_ptr & a){
	//return a->head-nodes.begin()>=nV;
	return arc_head(a)-nodes.begin()>=nV;
};
*/
...
template<class defs> void seed_hi_pr<defs>::global_update(){
	c2.resume();
...
//				for(arc * a = u->arc_first; a!=u->arc_end; ++a){//all outcoming arcs
				for(arc_ptr a = arc_first(u); allowed(a); ++a){//all outcoming arcs
					if(has_head(u,a) && arc_rev(a)->resCap>0){//has residual incoming arc
						node * v = arc_head(u,a);
						//assert(v->d != d_inf);
						if(v->d==d_inf+1 && v-this->nodes.begin()<this->nV){//non-boundary node, unlabelled
							v->d = b->d+1;
...
template<class defs> void seed_hi_pr<defs>::push1(node *u, arc_ptr & a, node *v){
	//v = arc_head(u,a);
	assert(v->d<d_inf);
	tcap f = a->resCap;
...
//! gap heuristic here
template<class defs> typename seed_hi_pr<defs>::tbucket * seed_hi_pr<defs>::gap(tbucket * b){
	//found a gap
...
//! global gap
template<class defs> void seed_hi_pr<defs>::global_gap(int gap){
	...
	*/
	...
	while(b->d >= gap){
		...
		for(node * i = b->first_inactive;i!=0;i=i->bnext){
			if(i-this->nodes.begin()<this->nV){//inner node
				buckets.iDelete(i);
				elliminate(i);
			};
		};
...
template<class defs> void seed_hi_pr<defs>::stageOne(bool reuse){
	if(!reuse){
		global_update();
	};
	//
...
template<class defs> void seed_hi_pr<defs>::dismiss_bucket(tbucket * a){
...
template<class defs> void seed_hi_pr<defs>::raise_seed(node *i, int d){
...
template<class defs> void seed_hi_pr<defs>::clear_seeds(){
...
template<class defs> void seed_hi_pr<defs>::update_seeds(){
	if(this->nB==0)return;//no seeds
...
template<class defs> void seed_hi_pr<defs>::allocate1(int nV, int nB, int nE, int nE_out, int *E_out, int d_inf, int S, int T){
	...
	BS.reserve(nB);
	//init nodes
	for(int v=nV;v<this->n;++v){//add to boundary list
		BS.push_back(&this->nodes[v]);
	};
};

template<class defs> tflow seed_hi_pr<defs>::cut_cost(int * C){
	return 0;
};
...
template<class defs> tflow seed_hi_pr<defs>::cut_cost(){
	...
	/*
	for(all_arc_ptr a = arc_first(); allowed(a);++a){
	if(has_head(a)){
	node * u = arc_tail(a);
	node * v = arc_head(a);
...
template<class defs> long long seed_hi_pr<defs>::size_required(int nV, int nB, int nE, int nE_out,int d_inf){
	long long r = 0;
	r+=sizeof(seed_hi_pr);
...
template class seed_hi_pr<>;
template class seed_hi_pr<seed_hi_pr_defs<base_graph_n> >;
