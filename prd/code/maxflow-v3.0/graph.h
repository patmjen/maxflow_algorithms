/* graph.h */
/* AS. Some insignificant modifications. */
...
public:
	...
	struct node
	{
		...
		//changed only if one of the fields above is changed
		...
		unsigned int			is_in_changed_list : 1; //!< set by maxflow if changed outside
		unsigned int			is_decided : 2; //!< this is for simplification in ARD
...
private:
	...
	public:
		...
		struct node_saved{
			node saved;
			node * dest;
			//node_saved (node *x):dest(x),saved(*x){};
		};
		struct arc_saved{
			arc saved;
			arc * dest;
			//arc_saved (arc *x):dest(x),saved(*x){};
		};
		...
		node				*queue_first[2], *queue_last[2];	//!< list of active nodes		
		int					TIME;								//!< monotonically increasing global counter
		//todo: save changed_list

		//internal data to restore correct trees, dont save this: 
		//nodeptr				*orphan_first, *orphan_last;		//!< list of pointers to orphans
		//DBlock<nodeptr>		*nodeptr_block; //!< storage for lists orphan_first, orphan_last
	};

public:
	graph_save gsave;
	void start_save(){
		//for(int i=0;i<gsave.nodes.size();++i){
		//	gsave.nodes[i].dest->is_saved = 0;
		//};
		//for(int i=0;i<gsave.arcs.size();++i){
		//	gsave.arcs[i].dest->is_saved = 0;
		//};
		gsave.nodes.clear();
		gsave.arcs.clear();
		gsave.flow = flow;
		//gsave.maxflow_iteration = maxflow_iteration;
		for(int i=0;i<2;++i){
			gsave.queue_first[i] = queue_first[i];
			gsave.queue_last[i] = queue_last[i];
		};
		gsave.TIME = TIME;
		gsave.started = true;
		gsave.nodes.reserve(1024);
		gsave.arcs.reserve(1024);
	};
	void stop_save(){
		gsave.started = false;
	};
	...
	void restore_node(node * dest, const node & s){
		dest->next = s.next;
		dest->parent = s.parent;
		dest->tr_cap = s.tr_cap;
		//
		dest->DIST = s.DIST;
		dest->TS = s.TS;
		//dest->is_marked = s.is_marked;
		dest->is_sink = s.is_sink;
		dest->is_marked = s.is_marked;
		dest->is_saved = 0;
	};
	...
	void restore_saved(){
		for(int i=0;i<gsave.nodes.size();++i){
			restore_node(gsave.nodes[i].dest,gsave.nodes[i].saved);
		};
		for(int i=0;i<gsave.arcs.size();++i){
			restore_arc(gsave.arcs[i].dest,gsave.arcs[i].saved);
		};
		flow = gsave.flow;
		//maxflow_iteration = gsave.maxflow_iteration;
		for(int i=0;i<2;++i){
			queue_first[i] = gsave.queue_first[i];
			queue_last[i] = gsave.queue_last[i];
		};
		TIME = gsave.TIME;
	};

	void save_node(node * x){
		//gsave.nodes.push_back(graph_save::node_saved(x));
		typename graph_save::node_saved & ns = gsave.nodes.push_back();
		ns.dest = x;
		ns.saved.next = x->next;
		ns.saved.parent = x->parent;
		ns.saved.tr_cap = x->tr_cap;
		ns.saved.DIST = x->DIST;
		ns.saved.TS = x->TS;
		ns.saved.is_sink = x->is_sink;
		ns.saved.is_marked = x->is_marked;
	};

	void save_arc(arc * x){
		//gsave.arcs.push_back(graph_save::arc_saved(x));
		typename graph_save::arc_saved & as = gsave.arcs.push_back();
		as.dest = x;
		as.saved.r_cap = x->r_cap;
	};

	void save(node * x){
		return;
		if(!x->is_saved && gsave.started){
			save_node(x);
			x->is_saved = 1;
		};
	};
