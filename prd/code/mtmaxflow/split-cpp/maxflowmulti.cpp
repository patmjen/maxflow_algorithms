				     ...
				     const mxArray	*prhs[]		/* mxArray input pointer array */)
{
	...
	//
	// Read the split parameters and the margin
	//
    int margin;
    bool use_multithread = 1;
	int nSplit;
	int split[10]; 
	int nThreads;
    if (nrhs > 3) {
		const mxArray *mxsplit = prhs[2];
		const mxArray *mxmargin = prhs[3];
		
		if ( !mxIsDouble(mxmargin) || mxGetNumberOfElements(mxmargin) != 1 ) {
			mexErrMsgTxt("Margin must be a double scalar");
		}
		margin  = int(*(double*)mxGetPr(mxmargin) + 0.5);
		if (margin<=0) {
			mexErrMsgTxt("Invalid margin");
		}
		
		
        nSplit = mxGetNumberOfElements(mxsplit);
		if ( nSplit <= 0 || nSplit > 9 ) {
			mexErrMsgTxt("Invalid number of threads");
		}

		double* psplit = mxGetPr(mxsplit);
		
		for (int i=0; i<nSplit; ++i) {
			//N.B. Convert from MATLAB to C indices
			split[i] = psplit[i] - 1; 
		}
		...
		//We use the normal BK-cut algorithm
        use_multithread = 0;
		mexErrMsgTxt("Need to specify split position and margin");
    }
	
	
	bool bidirectional = 0;
	if (nrhs >= 5) {
        const mxArray* mxbidir = prhs[4];
	...
	ParallelGraph<Type,Type,Type> pgraph(nThreads, n, /*guess*/ 5*n, split, margin );
	...
	double time_taken = pgraph.time_taken;
	double is_global = pgraph.is_global?1:0;
