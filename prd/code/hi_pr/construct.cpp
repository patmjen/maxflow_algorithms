...
//AS: added destructor for hi_pr


template<typename tcap> void hi_pr::construct(int _nV, int _nE, const int * _E, const tcap * _cap, const tcap * _excess){
...
template void hi_pr::construct<int>(int, int, const int *, const int *, const int *);
...
void inline hi_pr::add_arc(int tail, int head, cType cap1, cType cap2, long *& arc_first, long *& arc_tail, long & pos_current, arc *& arc_current, node *& nodes, long & node_min, long & node_max, long & no_alines){
...
void hi_pr::construct(const char * filename){
