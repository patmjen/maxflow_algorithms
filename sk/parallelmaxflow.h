...
constexpr int max_num_threads = 16;
...
public:
	...
	void update_graph(int s, int gpos, int iter, 
		signed char diff, signed char& prev_diff, captype& step, bool& has_flipped);
...
// 
template <typename captype, typename tcaptype, typename flowtype>
ParallelGraph<captype, tcaptype, flowtype>::ParallelGraph(
	int nThreads, int n, int edges, int split[], int margin, int * nnE) :
	graph(nThreads)
{
	...
	for (int g = 0; g < nThreads; ++g) {
		...
		graph[g] = graph_ptr(new GraphType(est_nodes, est_edges));
...
void ParallelGraph<captype, tcaptype, flowtype>::add_edge(int i, int j, captype v, captype vret)
{
	...
	for (int g = 0; g < nThreads; ++g) {
		if ((i < offset[g + 1] + margin && j < offset[g + 1] + margin && i >= offset[g] && j >= offset[g]) ||
			(g == 0 && i < margin && j < margin)) {
			//Within this graph
...
void ParallelGraph<captype, tcaptype, flowtype>::add_tweights(int i, tcaptype source, tcaptype sink)
{
	//Not the most efficient solution
	for (int g = 0; g < nThreads; ++g) {
		if ((i < offset[g + 1] + margin && i >= offset[g]) || (g == 0 && i < margin)) {
			//Within this graph
		...
		for (int s = 0; s < nSplit; ++s) {
			...
			for (int i = offset[s + 1]; i < offset[s + 1] + margin; ++i) {
				...
				if (l1 != l2) {
					...
					update_graph(s, i, iter, diff, prev_diff[s][i - offset[s + 1]], 
						steps[s][i - offset[s + 1]], has_flipped[s][i - offset[s + 1]]);
					nDiff++;
				}
			}
...
template <typename captype, typename tcaptype, typename flowtype>
void ParallelGraph<captype, tcaptype, flowtype>::update_graph(
	int s, int i, int iter, signed char diff, signed char& prev_diff, captype& step, bool& has_flipped)
{
	captype damping = 0.5;
